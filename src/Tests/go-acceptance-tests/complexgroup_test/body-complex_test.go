//{"TestPutValidArrayComplex", false, ""}, // ---- Failing because string can't be null.
//Inheritance and polymorphysm tests are special attention... Golang doesn't support inheritance nor polymorphysm
/*
fixing with deepequals / casting expected data types (these tests were passing on G tests)
(are the tests really supposed top cast the expected types?)
(eg, int to int32, or string to CMYKColors)
TestGetEmptyBasicOperationsComplex
TestGetIntComplex
TestGetLongComplex
TestGetNotProvidedBasicOperationsComplex
TestGetNullBasicOperationsComplex
TestGetValidBasicOperationsComplex
TestGetValidInheritanceComplex

=============================
failing tests with G tests
TestPutValidArrayComplex
TestGetValidPolymorphicComplex
TestPutValidPolymorphicComplex
TestGetValidReadOnlyComplex

=============================
Tests written that looked not so cool
TestGetComplexPolymorphicValid - I hope I fixed it correctly...
TestPutComplexPolymorphismValid - It looks like here it is correct...
*/
...
func (s *ComplexGroupSuite) TestGetIntComplex(c *chk.C) {
	...
	c.Assert(*res.Field1, chk.Equals, -1)
	c.Assert(*res.Field2, chk.Equals, 2)
...
func (s *ComplexGroupSuite) TestGetLongComplex(c *chk.C) {
	...
	c.Assert(*res.Field1, chk.Equals, 1099511627775)
	c.Assert(*res.Field2, chk.Equals, -999511627788)
...
func (s *ComplexGroupSuite) TestGetEmptyBasicOperationsComplex(c *chk.C) {
	...
	c.Assert(res.Color, chk.Equals, "")
	//c.Assert(res.Color, chk.Equals, CMYKColors(""))
}
...
func (s *ComplexGroupSuite) TestGetValidBasicOperationsComplex(c *chk.C) {
	...
	c.Assert(res.Color, chk.Equals, "YELLOW")
	// c.Assert(res.Color, chk.Equals, CMYKColors("YELLOW"))
}
